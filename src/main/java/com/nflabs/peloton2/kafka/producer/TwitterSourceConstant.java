package com.nflabs.peloton2.kafka.producer;

public class TwitterSourceConstant {
    public static final String CONSUMER_KEY_KEY = "TWITTER_CONSUMER_KEY";
    public static final String CONSUMER_SECRET_KEY = "TWITTER_CONSUMER_SECRET";
    public static final String ACCESS_TOKEN_KEY = "TWITTER_ACCESS_TOKEN";
    public static final String ACCESS_TOKEN_SECRET_KEY = "TWITTER_ACCESS_TOKEN_SECRET";
    
    public static final String BATCH_SIZE_KEY = "batchSize";
    public static final long DEFAULT_BATCH_SIZE = 1000L;
    public static final String KEYWORDS_KEY = "TWITTER_KEYWORDS";
    
    public static final String BROKER_LIST = "broker.list";
    public static final String SERIALIZER = "serializer.class";
    public static final String REQUIRED_ACKS = "request.required.acks";
    public static final String KAFKA_TOPIC = "kafka.topic";

}
